...
The observer can also be the wrong repository. In a fork with both `origin` and `upstream`, `gh` picks the base repository by remote priority and prefers `upstream`, so `gh run list`, `gh run watch`, `gh workflow run` — and `ci.sh`, which wraps them — report on the parent without a warning. Run `gh repo set-default OWNER/FORK` once, or pass `--repo`, and trust a verdict only when the run's `headSha` matches `git rev-parse HEAD`

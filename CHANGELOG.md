...
- `check-pins.sh`, `check-interface.sh`, `vendor-sync.sh` and `ci.sh` print their help from a heredoc instead of reading their own header back, which under `bash <(…)` is the pipe bash reads the script from and printed nothing; the network and bash claims and the note on where a file comes from stay in the header comment and are no longer part of `--help`
- `check-pins.sh` and `check-interface.sh`'s `--help` now carries what a caller needs, the network line included — `Nothing here reaches the network` — which the entry above said stayed in the header; the header keeps only the bash floor and the note on where the file comes from, which an editor needs and a caller does not. `check-templates.sh`, this repository's own gate, now answers `-h`, `--help` and `help` too
...
- `references/bump-cascade.md` says how a copy changes its source: its lock line deleted by hand and the path added again from the new source, with the old source keeping its file until every consumer has moved. It also says that `update` can take more copies than the one aimed at, so every copy it names is staged with the lock
- `references/bump-cascade.md` says when GitHub registers a vendored workflow, only on a push to the default branch that changes the file, and how to register one that never appeared under Actions
...
- `references/bump-cascade.md` says the cascade's own workflow is taken as a manual line of the lock, and that a repository whose build workflow goes by another name renames it to `build.yml` rather than keeping a hand-edited `vendor-sync.yml`: `uses:` takes only a literal, and the first rollout had left eleven such copies one line apart
- this repository's own workflow is `build.yml`, and declares `workflow_call` with a `ref`, as its template asks of everyone else; the readme points at `references/pinning.md` and `references/checks.md` for what the two checkers catch instead of retelling it

### Fixed

- **`vendor-sync.sh update --manual` took every line, not only the manual ones**, so a person refreshing a workflow copy by hand also landed every other copy's new content without the verify step. It takes the manual lines and only those now, and `update` without it names each manual copy whose source has moved — as a `::warning::` inside Actions — where it used to skip them in silence, so a manual copy could fall behind forever
- `vendor-sync.sh add` let a workflow file in as an ordinary line when its path began with `./`, or when the copy was a directory holding `.github/workflows/`; either way the weekly push would have been refused
- a vendored directory with a name git quotes read as edited in place right after it was taken, and one holding a symlink could never match its lock. Names are compared raw on both sides, and a symlink is refused when the copy is taken
- the templates had fallen behind the cascade: `vendor-sync.yml` kept its `uses:` line inside an `EXAMPLE` marker, inviting the one edit its consumers' gates refuse; `build.yml` had no `vendor-sync.sh check` step and still called `check-pins.sh` copied; `bump-cascade.yml` landed on a hardcoded `master`, so a repository whose default branch is `main` would have grown a `master` branch instead
...
- a trailing full stop on every rule in `SKILL.md` and on 58 paragraphs and list items of the references, against the rule every readme in the family is held to

...
- **Every check is proven able to fail.** A new check runs red first — against the pre-fix state or a deliberately broken fixture; checkers ship self-tests against known-bad inputs; assertions on generated text match whole lines, not substrings. A checker that has never been red is a decoration. Two reusable checkers ship as skeletons — `templates/no-secrets.sh` (refuse to ship a value that slipped past `.gitignore`) and `templates/check-skill.sh` (the gate a skill repository needs: SKILL.md loads at all, every file in `references/` is pointed at by it, every relative link and heading anchor resolves — each half proven able to fail); a copy of either must be falsified in its own repo, because the mechanism travels and the knowledge does not. Asking the same question of a *test suite* — would it notice if the code broke — belongs to the [tests](https://github.com/rokokol/tests-skill) skill and lives there. See [references/checks.md](references/checks.md).

...
- **Every check is proven able to fail.** A new check runs red first — against the pre-fix state or a deliberately broken fixture; checkers ship self-tests against known-bad inputs; assertions on generated text match whole lines, not substrings. A checker that has never been red is a decoration. Two reusable checkers ship as skeletons — `templates/falsify.py` (break one line of the implementation at a time, require the suite to notice) and `templates/no-secrets.sh` (refuse to ship a value that slipped past `.gitignore`); a copy of either must be falsified in its own repo, because the mechanism travels and the knowledge does not. See [references/checks.md](references/checks.md).
